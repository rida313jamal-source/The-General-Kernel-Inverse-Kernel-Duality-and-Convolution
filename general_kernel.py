...
# Build G(s) and P(s) symbolically based on template parameters
...
if st.button("Apply Kernel-Based Extraction"):
    ...
    if f_t is not None:
        try:
            from sympy import laplace_transform, apart
            L_f = laplace_transform(f_t, t_sym, s_sym, noconds=True)
            st.latex(r"\mathcal{L}\{f(t)\}(s) = " + sp.latex(L_f))

            # Simplify and compare with original F(s)
            # Compute partial fraction decomposition of L_f (should equal F(s))
            apart_result = apart(L_f, s_sym)
            st.markdown("**Partial fraction decomposition (coefficients extracted):**")
            st.latex(r"F(s) = " + sp.latex(apart_result))
            ...
            st.info("The coefficients can be read directly from the partial fraction decomposition of the original F(s).")
            # Still compute apart of F_s as a fallback
            try:
                apart_result = sp.apart(F_s, s_sym)
                st.latex(r"F(s) = " + sp.latex(apart_result))
            except:
                st.error("Could not perform partial fraction decomposition.")
    else:
        st.info("Proceed with the integral form. The forward transform can be computed manually.")
