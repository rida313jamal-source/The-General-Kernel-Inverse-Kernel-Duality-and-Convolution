...
def render_unified_framework_section():
    ...
    **Remark 6.** Unlike multiplication by $s$, division by $s$ produces no boundary term: there is no integration by parts, hence no evaluation at $t = 0$ or $t = \infty$. Therefore, the integral-injection law is boundary-safe and depends only on the integrability needed to justify the interchange of integrals.
    ...
    st.markdown("**I. Basic Generated Kernel Operations**")

    st.markdown("""
    | $s$-Domain Operator | $t$-Domain Spatial Representation | Analytical Classification |
    |---|---|---|
    | $T^{-1}\\{G(s)\\}$ | $g(t)$ | Base Generated Kernel |
    | $T^{-1}\\left\\{ \\frac{G(s)}{s} \\right\\}$ | $\\int_0^t g(u) du$ | Single Integral Injection |
    | $T^{-1}\\left\\{ \\frac{G(s)}{s^n} \\right\\}$ | $\\frac{1}{(n-1)!} \\int_0^t (t-\\tau)^{n-1} g(\\tau) d\\tau$ | $n$-Fold Repeated Integration |
    | $T^{-1}\\{G(s-a)\\}$ | $e^{at} g(t)$ | Kernel Shifting Dynamics |
    """)
    ...
    | $s$-Domain Operator | $t$-Domain Spatial Representation | Analytical Classification |
    |---|---|---|
    | $T^{-1}\\{1\\}$ | $\\delta(t)$ | Dirac Delta Neutral Anchor |
    | $T^{-1}\\left\\{ \\frac{1}{s-a} \\right\\}$ | $e^{at}$ | Isolated Pure Shifted Pole |
    """)
    ...
    |---|---|---|
    | $T^{-1}\\{1\\}$ | $\\delta(t)$ | |
    | $T^{-1}\\left\\{ \\frac{1}{s-a} \\right\\}$ | $e^{at}$ | |
    | $T^{-1}\\left\\{ \\frac{1}{(s-a)^n} \\right\\}$ | $\\frac{t^{n-1}}{(n-1)!} e^{at}$ | Higher-Order Pure Shifted Monomial |
    | $T^{-1}\\left\\{ \\frac{1}{s(s-a)^n} \\right\\}$ | $\\frac{1}{(n-1)!} \\int_0^x u^{n-1} e^{au} du$ | Neutral Base Shifted Monomial |
    """)
    ...
    | $s$-Domain Operator | $t$-Domain Spatial Representation | Analytical Classification |
    |---|---|---|
    | $T^{-1}\\left\\{ \\frac{G(s)}{s^2+a^2} \\right\\}$ | $\\frac{1}{a} \\int_0^t \\sin(a(t-\\tau)) g(\\tau) d\\tau$ | Trigonometric Modulated Response |
    | $T^{-1}\\left\\{ \\frac{G(s)}{s^2-a^2} \\right\\}$ | $\\frac{1}{a} \\int_0^t \\sinh(a(t-\\tau)) g(\\tau) d\\tau$ | Hyperbolic Sinh Modulated Kernel |
    | $T^{-1}\\left\\{ \\frac{sG(s)}{s^2+a^2} \\right\\}$ | $\\int_0^t \\cos(a(t-\\tau)) g(\\tau) d\\tau$ | Trigonometric Modulated Response |
    | $T^{-1}\\left\\{ \\frac{sG(s)}{s^2-a^2} \\right\\}$ | $\\int_0^t \\cosh(a(t-\\tau)) g(\\tau) d\\tau$ | Hyperbolic Cosh Operator Layer |
    """)
    ...
    | $s$-Domain Operator | $t$-Domain Spatial Representation | Analytical Classification |
    |---|---|---|
    | $T^{-1}\\left\\{ \\frac{G(s)}{s^n(s+a)} \\right\\}$ | $\\frac{1}{(n-1)!} \\int_0^t (t-\\tau)^{n-1} e^{-a(t-\\tau)} g(\\tau) d\\tau$ | Exponential-Monomial Composite |
    | $T^{-1}\\left\\{ \\frac{G(s)}{s^n(s^2+a^2)} \\right\\}$ | $\\frac{1}{a(n-1)!} \\int_0^t (t-\\tau)^{n-1} \\sin(a(t-\\tau)) g(\\tau) d\\tau$ | Trigonometric-Monomial Extension |
    | $T^{-1}\\left\\{ \\frac{G(s)}{s^n(s-a)^n} \\right\\}$ | $\\frac{1}{(n-1)!} \\int_0^t \\left( \\int_0^{t-\\tau} u^{n-1} e^{au} du \\right) g(\\tau) d\\tau$ | Multi-Layered Shifted Integration |
    """)
    ...
    | $s$-Domain Operator | $t$-Domain Spatial Representation | Analytical Classification |
    |---|---|---|
    | $T^{-1}\\left\\{ \\frac{G(s)}{P(s)} \\right\\}$ | $\\int_0^t h(t-\\tau) g(\\tau) d\\tau$ | General Dual-Kernel Convolution |
    | where $h(t) = \\mathcal{T}^{-1} \\left\\{ \\frac{1}{P(s)} \\right\\}$ | | Higher-Order Polynomial Regularization |
    | $\\mathcal{T}^{-1}\\left\\{ \\frac{G(s)}{s^n P(s)} \\right\\}$ | | Continuous Fractional Domain |
    | $\\frac{1}{\\Gamma(\\alpha)} \\int_0^t (t - \\tau)^{\\alpha - 1} g(\\tau) d\\tau$ | | |
    | $\\frac{1}{P(s)} \\int_0^t (t - \\tau)^{\\alpha - 1} g(\\tau) d\\tau$ | | |
    """)
    ...
    **Recovering Transform Domain Components and Coefficients**
    ...
    st.subheader("Example 9.11 (Decomposition of Mixed Higher-Order Quadratic Kernels)")
    ...
    st.subheader("Detailed Application of the General Kernel Framework to a Third-Order Cluster Pole")
    ...
    **Combining:**
    ...
    st.subheader("Example 9.12 (Continued): Distinct Linear Poles - Detailed Convolution")

    st.markdown(r"""
    **2. Detailed Convolutional Integration**

    Invoking Theorem 3.5 (The Generalized Convolution Kernel Identity), the exact spatial time-domain response $f(t)$ is constructed directly through a single continuous convolution integral, bypassing algebraic factorization:
    """)
    st.latex(r"""
    f(t) = \int_0^t h(t - \tau)g(\tau) \, d\tau
    """)
    st.markdown(r"""
    Substituting the identified primary weights $h(t - \tau) = e^{4(t - \tau)} - e^{3(t - \tau)}$ and $g(\tau) = e^{\tau}$ yields the following integral form:
    """)
    st.latex(r"""
    f(t) = \int_0^t \left( e^{4(t - \tau)} - e^{3(t - \tau)} \right) e^{\tau} \, d\tau
    """)
    st.markdown(r"""
    We distribute the exponential drive $e^{\tau}$ and separate the linear operational layers:
    """)
    st.latex(r"""
    f(t) = \int_0^t \left( e^{4t - 4\tau + \tau} - e^{3t - 3\tau + \tau} \right) \, d\tau
    """)
    st.latex(r"""
    f(t) = \int_0^t e^{4t} e^{-3\tau} \, d\tau - \int_0^t e^{3t} e^{-2\tau} \, d\tau
    """)
    st.markdown(r"""
    Since the integration is carried out strictly with respect to the spatial parameter $\tau$, the functions of $t$ are factored out as constant scaling coefficients:
    """)
    st.latex(r"""
    f(t) = e^{4t} \int_0^t e^{-3\tau} \, d\tau - e^{3t} \int_0^t e^{-2\tau} \, d\tau
    """)
    st.markdown(r"""
    Evaluating each linear continuous exponential layer independently yields:
    """)
    st.latex(r"""
    f(t) = e^{4t} \left[ -\frac{1}{3} e^{-3t} \right]_0^t - e^{3t} \left[ -\frac{1}{2} e^{-2t} \right]_0^t
    """)
    st.markdown(r"""
    Applying the fundamental integration boundaries:
    """)
    st.latex(r"""
    f(t) = e^{4t} \left( -\frac{1}{3} e^{-3t} + \frac{1}{3} \right) - e^{3t} \left( -\frac{1}{2} e^{-2t} + \frac{1}{2} \right)
    """)
    st.markdown(r"""
    Distributing the external exponential structures and collecting terms reveals the closed-form response:
    """)
    st.latex(r"""
    f(t) = -\frac{1}{3} e^{t} + \frac{1}{3} e^{4t} + \frac{1}{2} e^{t} - \frac{1}{2} e^{3t}
    """)
    st.markdown(r"""
    Combining the similar coefficients for the baseline factor $e^{t}$ (where $-\frac{1}{3} + \frac{1}{2} = \frac{1}{6}$) resolves to:
    """)
    st.latex(r"""
    f(t) = \frac{1}{6} e^{t} - \frac{1}{2} e^{3t} + \frac{1}{3} e^{4t}
    """)

    st.markdown(r"""
    **3. Forward Transformation Loop and Coefficient Extraction**

    To automatically recover the transform-domain components without setting up simultaneous equations or matrix vector solves, we project the spatial function $f(t)$ back into the forward Laplace transform domain:
    """)
    st.latex(r"""
    \mathcal{L}\{f(t)\}(s) = \frac{1}{6} \mathcal{L}\{e^{t}\} - \frac{1}{2} \mathcal{L}\{e^{3t}\} + \frac{1}{3} \mathcal{L}\{e^{4t}\}
    """)
    st.markdown(r"""
    Substituting the standard linear pairs from the forward tables yields the automatic residue expansion:
    """)
    st.latex(r"""
    \frac{1}{(s - 1)(s - 3)(s - 4)} = \frac{1}{6(s - 1)} - \frac{1}{2(s - 3)} + \frac{1}{3(s - 4)}
    """)
    st.markdown(r"""
    By direct visual matching against the classical template $\frac{A}{s - 1} + \frac{B}{s - 3} + \frac{C}{s - 4}$, the exact decomposition constants stand exposed instantly:
    """)
    st.latex(r"""
    A = \frac{1}{6}, \quad B = -\frac{1}{2}, \quad C = \frac{1}{3}
    """)
    st.markdown(r"""
    This perfectly demonstrates the invariant capability of the operational convolution route for distinct linear pole configurations.
    """)

    # ============================================================
    # Example 7: Fifth-Order Pole
    # ============================================================
    st.subheader("Example 7: Fifth-Order Pole")
    ...
    **2. Convolution Integration:**
    ...
    """)
    st.latex(r"""
    \int \tau^4 e^{-\tau}d\tau = -e^{-\tau}(\tau^4 + 4\tau^3 + 12\tau^2 + 24\tau + 24)
    """)
    st.latex(r"""
    f(t) = \frac{1}{24} \left[ -e^{-t}(t^4 + 4t^3 + 12t^2 + 24t + 24) - (-24) \right]
    """)
    st.latex(r"""
    f(t) = 1 - e^{-t} \left( 1 + t + \frac{t^2}{2} + \frac{t^3}{6} + \frac{t^4}{24} \right)
    """)
    ...
    **3. Forward Transform:**
    """)
    st.latex(r"""
    \mathcal{L}\{f(t)\} = \mathcal{L}\{1\} - \mathcal{L}\{e^{-t}\} - \mathcal{L}\{te^{-t}\} - \frac{1}{2}\mathcal{L}\{t^2e^{-t}\}
    """)
    st.latex(r"""
    - \frac{1}{6}\mathcal{L}\{t^3e^{-t}\} - \frac{1}{24}\mathcal{L}\{t^4e^{-t}\}
    """)
    st.latex(r"""
    = \frac{1}{s} - \frac{1}{s+1} - \frac{1}{(s+1)^2} - \frac{1}{(s+1)^3} - \frac{1}{(s+1)^4} - \frac{1}{(s+1)^5}
    """)
    ...
    **5. Coefficients:**
    ...
    st.header("10 chapter:3: A Unified Angular-Hyperbolic Representation of the Laplace Transform")

    st.markdown(r"""
    This part introduces a unified angular-hyperbolic representation of the Laplace transform, aimed at providing a geometric interpretation of Laplace kernels. The proposed framework distinguishes...
    """)
